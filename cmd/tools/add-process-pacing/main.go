...
func main() {
	...
	fmt.Println("Version clonada y actualizada correctamente")
...
func parseOptions() options {
	...
	flag.IntVar(&opts.PacingMessages, "pacing-messages", 100, "Cantidad de items por invocacion")
	flag.IntVar(&opts.PacingInterval, "pacing-interval", 2, "Delay entre auto_invoke (1..10)")
...
func validateOptions(opts options) error {
	...
	if opts.PacingMessages <= 0 {
		return fmt.Errorf("pacing_messages debe ser mayor a 0")
	}
	if opts.PacingInterval < 1 || opts.PacingInterval > 10 {
		return fmt.Errorf("pacing_interval debe estar entre 1 y 10")
	}
	return nil
}
