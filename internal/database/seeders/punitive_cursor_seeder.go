...
func BatchProcessPunitiveCursorSeeder(pool *pgxpool.Pool) error {
	...
	logger := slog.Default().With("seeder", "batch_process_punitive_cursor")
	...
	return executeInTransaction(ctx, pool, func(ctx context.Context, tx pgx.Tx) error {
		...
		steps := []struct {
			...
			},
			{
				...
				ExecutionKey: "bulk/process/punitive/dispatch_shards",
				Config:       `{"parallel_shards":1}`,
			},
			{
				Order:        3,
				Name:         "Step 3: Procesar paginas",
				ExecutionKey: "bulk/process/punitive/process_batch",
				Config: `{
					...
					"execution_policy": {
						"mode": "ASYNC",
						"label": "Punitorios cursor",
						"auto_invoke": {
							"enabled": true,
							"cursor_field": "batch_index",
							"stop_condition": "is_last_batch"
						},
						"next_step": "bulk/process/punitive/finalize"
					}
				}`,
			},
			{
				...
				ExecutionKey: "bulk/process/punitive/finalize",
				Config:       "{}",
			},
		}
