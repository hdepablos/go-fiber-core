...
func BatchProcessPunitivecursorSeeder(pool *pgxpool.Pool) error {
	ctx, cancel := context.WithTimeout(context.Background(), defaultSeederTimeout)
	defer cancel()

	logger := slog.Default().With("seeder", "batch_process_punitivecursor")
		...
		steps := []struct {
			...
			},
			{
				...
				Config:       `{
					...
					"execution_policy": {
						"mode": "ASYNC",
						"label": "Calcular los punitorios con cursor",
						"auto_invoke": {
							"enabled": true,
