...
func BulkProcessGenericFanoutSeeder(pool *pgxpool.Pool) error {
	ctx, cancel := context.WithTimeout(context.Background(), defaultSeederTimeout)
	defer cancel()

	logger := slog.Default().With("seeder", "bulk_process_generic_fanout")
	...
	return executeInTransaction(ctx, pool, func(ctx context.Context, tx pgx.Tx) error {
		...
		const processDescription = "Proceso batch genérico reutilizable con batchflow"
		...
		steps := []struct {
			...
			{
				Order:        2,
				Name:         "Step 2: Dispatch shards batchflow",
			...
			{
				Order:        3,
				Name:         "Step 3: Procesar lotes de registros",
				...
				Config: `{
					...
					"execution_policy": {
						"mode": "ASYNC",
						"label": "procesar lote generico fanout",
						"auto_invoke": {
							"enabled": true,
