...
func ExportManagerGenerarArchivoBancoGaliciaSeeder(pool *pgxpool.Pool) error {
	ctx, cancel := context.WithTimeout(context.Background(), defaultSeederTimeout)
	defer cancel()

	logger := slog.Default().With("seeder", "export_manager_generar_archivo_banco_galicia")
	...
	return executeInTransaction(ctx, pool, func(ctx context.Context, tx pgx.Tx) error {
		...
		steps := []struct {
		...
		}{
			{
				...
				ExecutionKey: "bulk/export/generar_archivo_banco_galicia/start",
				Config:       `{"batch_size":5000,"part_prefix":"exports/bulk_jobs/generar_archivo_banco_galicia","redis_ttl_hours":24}`,
			},
			{
				Order:        2,
				Name:         "Step 2: Procesar batch con header/body/footer",
				ExecutionKey: "bulk/export/generar_archivo_banco_galicia/process_batch",
				Config: `{
					"execution_policy": {
						"mode": "ASYNC",
						"label": "generar archivo banco galicia",
						"auto_invoke": {
							"enabled": true,
							"cursor_field": "batch_index",
							"stop_condition": "is_last_batch"
						},
						"next_step": "bulk/export/generar_archivo_banco_galicia/finalize"
					}
				}`,
			},
			{
				...
				ExecutionKey: "bulk/export/generar_archivo_banco_galicia/finalize",
				Config:       `{"file":"exports/bank/galicia/manager-galicia"}`,
			},
		}
