...
func BatchProcessPunitivecursorCursorSeeder(pool *pgxpool.Pool) error {
	ctx, cancel := context.WithTimeout(context.Background(), defaultSeederTimeout)
	defer cancel()

	logger := slog.Default().With("seeder", "batch_process_punitivecursor_cursor")
	logger.Info("iniciando seeder")

	return executeInTransaction(ctx, pool, func(ctx context.Context, tx pgx.Tx) error {
		const processTypeName = "Calcular los punitorios con cursor"
		...
		steps := []struct {
		...
		}{
			{
				...
				ExecutionKey: "bulk/process/punitivecursor/start",
				Config:       `{"batch_size":500,"redis_ttl_hours":24,"source_mode":"cursor"}`,
			},
			{
				Order:        2,
				Name:         "Step 2: Continuar proceso secuencial",
				ExecutionKey: "bulk/process/punitivecursor/dispatch_shards",
				Config:       `{"parallel_shards":1}`,
			},
			{
				Order:        3,
				Name:         "Step 3: Procesar paginas",
				ExecutionKey: "bulk/process/punitivecursor/process_batch",
				Config:       `{
					...
					"execution_policy": {
						"mode": "ASYNC",
						"label": "Calcular los punitorios con cursor cursor",
						"auto_invoke": {
							"enabled": true,
							"cursor_field": "batch_index",
							"stop_condition": "is_last_batch"
						},
						"next_step": "bulk/process/punitivecursor/finalize"
					}
				}`,
			},
			{
				...
				ExecutionKey: "bulk/process/punitivecursor/finalize",
				Config:       "{}",
			},
		}
