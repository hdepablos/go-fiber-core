...
type bulkJobFinalizer struct {
	readDB *gorm.DB
}

func NewBulkJobFinalizer(readDB *gorm.DB) batchflow.Finalizer {
	return &bulkJobFinalizer{readDB: readDB}
}

func (f *bulkJobFinalizer) Finalize(ctx context.Context, execCtx batchflow.ExecutionContext, req batchflow.FinalizeRequest) (batchflow.FinalizeResult, error) {
	var rows []struct {
